...
import (
	...
	"github.com/abhinavmedikonda/OMS/account/pb"
...
func NewClient(url string) (*Client, error) {
	conn, err := grpc.NewClient(
		url,
		append([]grpc.DialOption{grpc.WithTransportCredentials(insecure.NewCredentials())}, observability.GRPCClientOptions()...)...,
	)
	if err != nil {
		return nil, err
	}
...
func (c *Client) ListAccounts(ctx context.Context, skip uint64, take uint64) ([]Account, error) {
	r, err := c.service.ListAccounts(ctx, &pb.ListAccountsRequest{Skip: skip, Take: take})
	if err != nil {
		return nil, err
	}
	accounts := []Account{}
